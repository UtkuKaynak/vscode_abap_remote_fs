...
export class AbapDebugSession extends LoggingDebugSession {
  ...
  protected async setVariableRequest(
    ...
    const { value, success } = await this.listener.variableManager.setVariable(
  ...
  protected async variablesRequest(
    ...
    response.body = {
      variables: await this.listener.variableManager.getVariables(args.variablesReference)
    }
    this.sendResponse(response)
  }
  ...
  protected async gotoRequest(
    ...
    if (target?.instructionPointerReference) {
      try {
        const service = this.listener.service(args.threadId)
        await service.debuggerStep(
          "stepJumpToLine",
          args.threadId,
          target.instructionPointerReference
        )
        response.success = true
      } catch (error) {
        response.message =
          errorType(error) === "stepNotPossible"
            ? "Can't jump to this location"
      ...
      if (args.source?.path) {
        ...
        if (isAbapFile(n)) {
          ...
          const target = {
            ...
            line: args.line,
            instructionPointerReference: url
          }
